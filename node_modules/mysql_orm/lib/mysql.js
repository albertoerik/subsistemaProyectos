...
DEPENDENCIA
	...
	puedes consultar tablas de muchos a muchos
...
var db=function(conf)
{
	...
	this.get=function(table)
	{
		...
		var process_query=function(content,rows,callback){
			var connection=parent.connect();
			
			//console.log(keys_table1);
			if(content.parent==null)
			{
				keys_table1=Object.keys(content);
				var objparent=content;
			}else{
				keys_table1=Object.keys(content.parent);
				var objparent=content.parent;	
			}
			...
			if(rows.result.length==0){
					...
					var tablerel="";
					keys_table2=Object.keys(rows.result[0]);
					var table1=objparent["idRel"];
					var table2=rows.result[0]["idRel"];
					var indice=0;
					var indicej=0
					...
					//Determino la llave primaria
					connection.query("show KEYS from "+jointable,[],function(err,rr,fields){
						var query_relation="select fi.* from "+tablerel+" as rel, "+jointable+" as fi where rel."+parentkey+"="+objparent.id+" and rel."+joinkey+"=fi."+rr[0].Column_name;
						//console.log(query_relation);
						
						connection.query(query_relation,[],function(err,dataDB,fields){
							objparent[jointable]=dataDB;
							callback(objparent);
							connection.end();;
						});
					});
		...
		this.all=function(callback){
			...
			{
				var content=this;
				var information=content.colletion;
				parent.get(this.name).limit(1).execute(function(rows){
					if(content.parent==null)
					{
						
						var c=0;
						for(var i=0;i<information.result.length;i++)
						{
							process_query(information.result[i],rows,function(r){
								c++;
								if(i==c){
									callback({load:true});	
								}
						...
						process_query(content,rows,function(r){
							callback(r);
						});
					}
				...
				var obj=Object();
				if(this.parent==null){
					var data=this.colletion.result;
					var c=0;
					for(var i=0;i<data.length;i++)
					{
						data[i][this.name].all(function(r){
							c++;
							if(i==c)
							{
		...
		this.execute=function(callback)
		{
			...
			var parent=this;
			connection.query(sql,[],function(err,r,fields){
				main_rows={"result":r};
				...
				schema_connect.query(query_relation,[],function(err,rows,fields){
					...
					for(var j in main_rows.result)
					{
						main_rows.result[j]['idRel']=Array();
						...
						for(var i in rows)
						{
							...
							main_rows.result[j][table_rel]=Array();
							main_rows.result[j][table_rel].all=parent.all;
							main_rows.result[j][table_rel].name=table_rel;
							main_rows[table_rel]=Array();
							main_rows[table_rel].all=parent.all;
							main_rows[table_rel].name=table_rel;
							main_rows[table_rel].colletion=main_rows;
							var obj=Object();
							obj[table_rel]=id_rel;
							main_rows.result[j]['idRel'].push(obj);
							main_rows.result[j]['idRef']=(id_ref);
							main_rows.result[j][table_rel].parent=main_rows.result[j];

							if(jointable!="")
							{	//console.log(main_rows[j])
								main_rows.result[j][jointable]=Array();
								main_rows.result[j][jointable].all=parent.all;
								main_rows.result[j][jointable].name=jointable;
								main_rows.result[j][jointable].parent=main_rows.result[j];
								main_rows[jointable]=Array();
								main_rows[jointable].all=parent.all;
								main_rows[jointable].name=jointable;
								main_rows[jointable].colletion=main_rows;
							}	
						}
					}
